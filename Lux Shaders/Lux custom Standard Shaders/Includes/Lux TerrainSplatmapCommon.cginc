...
struct Input
{
	...
	// Lux
	float4 lux_worldPosDistance : TEXCOORD6;
	float3 viewDir;
	float3 worldNormal;
	INTERNAL_DATA
};
...
#endif
{
	...
	mixedDiffuse = 0.0f;
		...
		mixedDiffuse += splat_control.a * tex2D(_Splat3, IN.uv_Splat2) * half4(1.0, 1.0, 1.0, defaultAlpha.a);
	#else
		...
		mixedDiffuse += splat_control.a * tex2D(_Splat3, IN.uv_Splat2);
	#endif
		...
		nrm += splat_control.a * tex2D(_Normal3, IN.uv_Splat2);
